...
void Task_OpenMainMenu(u8 taskId)
{
    ...
    {   
        ...
        {                //  where the UI is initialized by swapping a task func with this one 
            ...
            default:
                SetMainCallback2(CB2_NewGameFrankSpeech_FromNewMainMenu);
                DestroyTask(taskId);
                return;
            case HAS_SAVED_GAME:       
...
static void MainMenu_VBlankCB(void)
{
    ...
    TransferPlttBuffer();
    ChangeBgY(2, 64, BG_COORD_SUB); // This controls the scrolling of the scroll bg - reduced from 128 to 32 for slower scroll
}
    ...
    if (JOY_NEW(A_BUTTON)) // If Pressed A go to thing you pressed A on
    {
        ...
        switch(sSelectedOption)
        {
            ...
            case HW_WIN_NEW_GAME:
                sMainMenuDataPtr->savedCallback = CB2_NewGameFrankSpeech_FromNewMainMenu;
                sSelectedOption = HW_WIN_CONTINUE;
                break;
            case HW_WIN_OPTIONS:
