 ...
 *  - If the user selected New Game, advance to Task_NewGameFrankSpeech_Init.
 ...
 *
 * Task_NewGameFrankSpeech_Init
 *  - Load the sprites for the intro speech, start playing music
 * Task_NewGameFrankSpeech_WaitToShowFrank
 *  - Spawn Task_NewGameFrankSpeech_FadeInTarget1OutTarget2
 *  - Spawn Task_NewGameFrankSpeech_FadePlatformOut
 *  - Both of these tasks destroy themselves when done.
 * Task_NewGameFrankSpeech_WaitForSpriteFadeInWelcome
 * Task_NewGameFrankSpeech_ThisIsAPokemon
 *  - When the text is done printing, spawns Task_NewGameFrankSpeechSub_InitPokeball
 * Task_NewGameFrankSpeech_MainSpeech
 * Task_NewGameFrankSpeech_AndYouAre
 * Task_NewGameFrankSpeech_StartFrankLotadPlatformFade
 * Task_NewGameFrankSpeech_StartFrankLotadPlatformFade
 * Task_NewGameFrankSpeech_SlidePlatformAway
 * Task_NewGameFrankSpeech_StartPlayerFadeIn
 * Task_NewGameFrankSpeech_WaitForPlayerFadeIn
 * Task_NewGameFrankSpeech_BoyOrGirl
 * Task_NewGameFrankSpeech_WaitToShowGenderMenu
 * Task_NewGameFrankSpeech_ChooseGender
 *  - Animates by advancing to Task_NewGameFrankSpeech_SlideOutOldGenderSprite
 *    whenever the player's selection changes.
 *  - Advances to Task_NewGameFrankSpeech_WhatsYourName when done.
 *
 * Task_NewGameFrankSpeech_SlideOutOldGenderSprite
 * Task_NewGameFrankSpeech_SlideInNewGenderSprite
 *  - Returns back to Task_NewGameFrankSpeech_ChooseGender.
 *
 * Task_NewGameFrankSpeech_WhatsYourName
 * Task_NewGameFrankSpeech_WaitForWhatsYourNameToPrint
 * Task_NewGameFrankSpeech_WaitPressBeforeNameChoice
 * Task_NewGameFrankSpeech_StartNamingScreen
 * C2_NamingScreen
 *  - Returns to CB2_NewGameFrankSpeech_ReturnFromNamingScreen when done
 * CB2_NewGameFrankSpeech_ReturnFromNamingScreen
 * Task_NewGameFrankSpeech_ReturnFromNamingScreenShowTextbox
 * Task_NewGameFrankSpeech_SoItsPlayerName
 * Task_NewGameFrankSpeech_CreateNameYesNo
 * Task_NewGameFrankSpeech_ProcessNameYesNoMenu
 *  - If confirmed, advance to Task_NewGameFrankSpeech_SlidePlatformAway2.
 *  - Otherwise, return to Task_NewGameFrankSpeech_BoyOrGirl.
 *
 * Task_NewGameFrankSpeech_SlidePlatformAway2
 * Task_NewGameFrankSpeech_ReshowFrankLotad
 * Task_NewGameFrankSpeech_WaitForSpriteFadeInAndTextPrinter
 * Task_NewGameFrankSpeech_AreYouReady
 * Task_NewGameFrankSpeech_ShrinkPlayer
 * Task_NewGameFrankSpeech_WaitForPlayerShrink
 * Task_NewGameFrankSpeech_FadePlayerToWhite
 * Task_NewGameFrankSpeech_Cleanup
 *  - Advances to CB2_NewGame.
 *
 * Task_NewGameFrankSpeechSub_InitPokeball
 *  - Advances to Task_NewGameFrankSpeechSub_WaitForLotad
 * Task_NewGameFrankSpeechSub_WaitForLotad
...
static void Task_NewGameFrankSpeech_Init(u8);
static void Task_DisplayMainMenuInvalidActionError(u8);
static void AddFrankSpeechObjects(u8);
static void Task_NewGameFrankSpeech_WaitToShowFrank(u8);
static void NewGameFrankSpeech_StartFadeInTarget1OutTarget2(u8, u8);
...
static void NewGameFrankSpeech_StartFadePlatformIn(u8, u8);
static void LoadMainMenuWindowFrameTiles(u8, u16);
static void DrawMainMenuWindowBorder(const struct WindowTemplate *, u16);
static void Task_HighlightSelectedMainMenuItem(u8);
static void Task_NewGameFrankSpeech_ChooseGender(u8);
static s8 NewGameFrankSpeech_ProcessGenderMenuInput(void);
static void NewGameFrankSpeech_ClearGenderWindow(u8, u8);
static void Task_NewGameFrankSpeech_WhatsYourName(u8);
static void Task_NewGameFrankSpeech_SlideOutOldGenderSprite(u8);
static void Task_NewGameFrankSpeech_SlideInNewGenderSprite(u8);
static void Task_NewGameFrankSpeech_WaitForWhatsYourNameToPrint(u8);
static void Task_NewGameFrankSpeech_WaitPressBeforeNameChoice(u8);
static void Task_NewGameFrankSpeech_StartNamingScreen(u8);
static void CB2_NewGameFrankSpeech_ReturnFromNamingScreen(void);
static void Task_NewGameFrankSpeech_SoItsPlayerName(u8);
void CreateYesNoMenuParameterized(u8, u8, u16, u16, u8, u8);
static void Task_NewGameFrankSpeech_SlidePlatformAway2(u8);
...
static void Task_NewGameFrankSpeech_FadePlayerToWhite(u8);
static void Task_NewGameFrankSpeech_Cleanup(u8);
static void SpriteCB_Null(struct Sprite *);
static void Task_NewGameFrankSpeech_ReturnFromNamingScreenShowTextbox(u8);
static void MainMenu_FormatSavegamePlayer(void);
static void MainMenu_FormatSavegamePokedex(void);
static void MainMenu_FormatSavegameTime(void);
static void MainMenu_FormatSavegameBadges(void);
static void NewGameFrankSpeech_CreateDialogueWindowBorder(u8, u8, u8, u8, u8, u8);

// .rodata

static const u16 sFrankSpeechBgPals[][16] = {
    INCBIN_U16("graphics/frank_speech/bg0.gbapal"),
    INCBIN_U16("graphics/frank_speech/bg1.gbapal")
};

static const u32 sFrankSpeechShadowGfx[] = INCBIN_U32("graphics/frank_speech/shadow.4bpp.smol");
static const u32 sFrankSpeechBgMap[] = INCBIN_U32("graphics/frank_speech/map.bin.smolTM");
static const u16 sFrankSpeechBgGradientPal[] = INCBIN_U16("graphics/frank_speech/bg2.gbapal");
...
static const struct WindowTemplate sNewGameFrankSpeechTextWindows[] =
{
    {
        .bg = 0,
        .tilemapLeft = 2,
...
static const struct BgTemplate sFrankBgTemplate = {
    .bg = 0,
    .charBaseIndex = 3,
    .mapBaseIndex = 30,
    .screenSize = 0,
...
#define FRANK_DLG_BASE_TILE_NUM 0xFC
    ...
    if (!gPaletteFade.active)
    {
        ...
        switch (action)
        {
            ...
            default:
                ...
                gTasks[taskId].func = Task_NewGameFrankSpeech_Init;
                break;
...
#define tFrankSpriteId data[8]
#define tLotadSpriteId data[9]
#define tBrendanSpriteId data[10]
#define tMaySpriteId data[11]

static void Task_NewGameFrankSpeech_Init(u8 taskId)
{
    SetGpuReg(REG_OFFSET_DISPCNT, 0);
    SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_OBJ_ON | DISPCNT_OBJ_1D_MAP);
    InitBgFromTemplate(&sFrankBgTemplate);
    SetGpuReg(REG_OFFSET_WIN0H, 0);
    SetGpuReg(REG_OFFSET_WIN0V, 0);
    SetGpuReg(REG_OFFSET_WININ, 0);
    SetGpuReg(REG_OFFSET_WINOUT, 0);
    ...
    DecompressDataWithHeaderVram(sFrankSpeechShadowGfx, (void *)VRAM);
    DecompressDataWithHeaderVram(sFrankSpeechBgMap, (void *)(BG_SCREEN_ADDR(7)));
    LoadPalette(sFrankSpeechBgPals, BG_PLTT_ID(0), 2 * PLTT_SIZE_4BPP);
    LoadPalette(&sFrankSpeechBgGradientPal[8], BG_PLTT_ID(0) + 1, PLTT_SIZEOF(8));
    ScanlineEffect_Stop();
    ResetSpriteData();
    FreeAllSpritePalettes();
    ResetAllPicSprites();
...
void CB2_NewGameFrankSpeech_FromNewMainMenu(void) // Combination of the Above function and another to properly load the new game frank code from a seperate menu
{
    u8 taskId;
    ...
    LoadPalette(&sFrankSpeechBgGradientPal[8], BG_PLTT_ID(0) + 1, PLTT_SIZEOF(8));
    ResetTasks();
    taskId = CreateTask(Task_NewGameFrankSpeech_WaitToShowFrank, 0);
    gTasks[taskId].tBG1HOFS = 0;
    gTasks[taskId].tPlayerSpriteId = SPRITE_NONE;
    gTasks[taskId].data[3] = 0xFF;
    ...
    SetVBlankCallback(VBlankCB_MainMenu);
    SetMainCallback2(CB2_MainMenu);
    InitWindows(sNewGameFrankSpeechTextWindows);
...
static void Task_NewGameFrankSpeech_WaitToShowFrank(u8 taskId)
{
    u8 spriteId;
    ...
    {
        spriteId = gTasks[taskId].tFrankSpriteId;
        gSprites[spriteId].x = 136;
        gSprites[spriteId].y = 60;
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        NewGameFrankSpeech_StartFadeInTarget1OutTarget2(taskId, 0);
        NewGameFrankSpeech_StartFadePlatformOut(taskId, 0);
        gTasks[taskId].tTimer = 0;
        gTasks[taskId].func = Task_NewGameFrankSpeech_WaitForSpriteFadeInWelcome;
    }
}

static void Task_NewGameFrankSpeech_WaitForSpriteFadeInWelcome(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        gSprites[gTasks[taskId].tFrankSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        if (gTasks[taskId].tTimer)
        {
            gTasks[taskId].tTimer--;
        }
        else
        {
            InitWindows(sNewGameFrankSpeechTextWindows);
            LoadMainMenuWindowFrameTiles(0, 0xF3);
            LoadMessageBoxGfx(0, FRANK_DLG_BASE_TILE_NUM, BG_PLTT_ID(15));
            NewGameFrankSpeech_ShowDialogueWindow(0, 1);
            PutWindowTilemap(0);
            CopyWindowToVram(0, COPYWIN_GFX);
            NewGameFrankSpeech_ClearWindow(0);
            StringExpandPlaceholders(gStringVar4, gText_Frank_Welcome);
            AddTextPrinterForMessage(TRUE);
            gTasks[taskId].func = Task_NewGameFrankSpeech_ThisIsAPokemon;
        }
    }
}

static void Task_NewGameFrankSpeech_ThisIsAPokemon(u8 taskId)
{
    if (!gPaletteFade.active && !RunTextPrintersAndIsPrinter0Active())
    {
        gTasks[taskId].func = Task_NewGameFrankSpeech_MainSpeech;
        StringExpandPlaceholders(gStringVar4, gText_ThisIsAPokemon);
        AddTextPrinterWithCallbackForMessage(TRUE, NewGameFrankSpeech_WaitForThisIsPokemonText);
        sFrankSpeechMainTaskId = taskId;
    }
}

static void Task_NewGameFrankSpeech_MainSpeech(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        StringExpandPlaceholders(gStringVar4, gText_Frank_MainSpeech);
        AddTextPrinterForMessage(TRUE);
...
static void Task_NewGameFrankSpeechSub_InitPokeBall(u8 taskId)
{
    u8 spriteId = gTasks[sFrankSpeechMainTaskId].tLotadSpriteId;
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, PALETTES_BG, SPECIES_RATTATA_ALOLA);
    gTasks[taskId].func = Task_NewGameFrankSpeechSub_WaitForLotad;
    gTasks[sFrankSpeechMainTaskId].tTimer = 0;
}

static void Task_NewGameFrankSpeechSub_WaitForLotad(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
    struct Sprite *sprite = &gSprites[gTasks[sFrankSpeechMainTaskId].tLotadSpriteId];
    ...
    switch (tState)
    {
        case 0:
            ...
            break;
        case 1:
            if (gTasks[sFrankSpeechMainTaskId].tTimer >= 96)
            {
                DestroyTask(taskId);
                if (gTasks[sFrankSpeechMainTaskId].tTimer < 0x4000)
                    gTasks[sFrankSpeechMainTaskId].tTimer++;
            }
            return;
    }
    tState++;
...
static void Task_NewGameFrankSpeech_ChooseGender(u8 taskId)
{
    int gender = NewGameFrankSpeech_ProcessGenderMenuInput();
    int gender2;
    ...
    switch (gender)
    {
        case MALE:
            ...
            NewGameFrankSpeech_ClearGenderWindow(1, 1);
            gTasks[taskId].func = Task_NewGameFrankSpeech_WhatsYourName;
            break;
        case FEMALE:
            PlaySE(SE_SELECT);
            gSaveBlock2Ptr->playerGender = gender;
    ...
    if (gender2 != gTasks[taskId].tPlayerGender)
    {
        ...
        NewGameFrankSpeech_StartFadeOutTarget1InTarget2(taskId, 0);
        gTasks[taskId].func = Task_NewGameFrankSpeech_SlideOutOldGenderSprite;
    }
}

static void Task_NewGameFrankSpeech_SlideOutOldGenderSprite(u8 taskId)
{
    ...
    else
    {
        ...
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        NewGameFrankSpeech_StartFadeInTarget1OutTarget2(taskId, 0);
        gTasks[taskId].func = Task_NewGameFrankSpeech_SlideInNewGenderSprite;
    }
}

static void Task_NewGameFrankSpeech_SlideInNewGenderSprite(u8 taskId)
{
    u8 spriteId = gTasks[taskId].tPlayerSpriteId;
    ...
    else
    {
        ...
        if (gTasks[taskId].tIsDoneFadingSprites)
        {
            ...
            gTasks[taskId].func = Task_NewGameFrankSpeech_ChooseGender;
        }
    }
}

static void Task_NewGameFrankSpeech_WhatsYourName(u8 taskId)
{
    NewGameFrankSpeech_ClearWindow(0);
    StringExpandPlaceholders(gStringVar4, gText_Frank_WhatsYourName);
    AddTextPrinterForMessage(TRUE);
    gTasks[taskId].func = Task_NewGameFrankSpeech_WaitForWhatsYourNameToPrint;
}

static void Task_NewGameFrankSpeech_WaitForWhatsYourNameToPrint(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
        gTasks[taskId].func = Task_NewGameFrankSpeech_WaitPressBeforeNameChoice;
}

static void Task_NewGameFrankSpeech_WaitPressBeforeNameChoice(u8 taskId)
{
    if ((JOY_NEW(A_BUTTON)) || (JOY_NEW(B_BUTTON)))
    {
        BeginNormalPaletteFade(PALETTES_ALL, 0, 0, 16, RGB_BLACK);
        gTasks[taskId].func = Task_NewGameFrankSpeech_StartNamingScreen;
    }
}

static void Task_NewGameFrankSpeech_StartNamingScreen(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        FreeAllWindowBuffers();
        FreeAndDestroyMonPicSprite(gTasks[taskId].tLotadSpriteId);
        NewGameFrankSpeech_SetDefaultPlayerName(Random() % NUM_PRESET_NAMES);
        DestroyTask(taskId);
        DoNamingScreen(NAMING_SCREEN_PLAYER, gSaveBlock2Ptr->playerName, gSaveBlock2Ptr->playerGender, 0, 0, CB2_NewGameFrankSpeech_ReturnFromNamingScreen);
    }
}

static void Task_NewGameFrankSpeech_SoItsPlayerName(u8 taskId)
{
    // Skipped displaying gText_Frank_SoItsPlayer
    gTasks[taskId].func = Task_NewGameFrankSpeech_SlidePlatformAway2;
}

static void Task_NewGameFrankSpeech_SlidePlatformAway2(u8 taskId)
{
    ...
    {
        gTasks[taskId].func = Task_NewGameFrankSpeech_ReshowFrankLotad;
    }
}

static void Task_NewGameFrankSpeech_ReshowFrankLotad(u8 taskId)
{
    u8 spriteId;
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        StringExpandPlaceholders(gStringVar4, gText_Frank_YourePlayer);
        AddTextPrinterForMessage(TRUE);
        gTasks[taskId].func = Task_NewGameFrankSpeech_WaitForSpriteFadeInAndTextPrinter;
    }
}

static void Task_NewGameFrankSpeech_WaitForSpriteFadeInAndTextPrinter(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        gSprites[gTasks[taskId].tFrankSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        ...
        if (!RunTextPrintersAndIsPrinter0Active())
        {
            gSprites[gTasks[taskId].tFrankSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
            gSprites[gTasks[taskId].tLotadSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
            NewGameFrankSpeech_StartFadeOutTarget1InTarget2(taskId, 2);
            NewGameFrankSpeech_StartFadePlatformIn(taskId, 1);
...
static void Task_NewGameFrankSpeech_AreYouReady(u8 taskId)
{
    u8 spriteId;

    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        gSprites[gTasks[taskId].tFrankSpriteId].invisible = TRUE;
...
static void Task_NewGameFrankSpeech_ShrinkPlayer(u8 taskId)
{
    u8 spriteId;
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        if (!RunTextPrintersAndIsPrinter0Active())
        {
            ...
            FadeOutBGM(4);
            gTasks[taskId].func = Task_NewGameFrankSpeech_WaitForPlayerShrink;
        }
    }
}

static void Task_NewGameFrankSpeech_WaitForPlayerShrink(u8 taskId)
{
    u8 spriteId = gTasks[taskId].tPlayerSpriteId;

    if (gSprites[spriteId].affineAnimEnded)
        gTasks[taskId].func = Task_NewGameFrankSpeech_FadePlayerToWhite;
}

static void Task_NewGameFrankSpeech_FadePlayerToWhite(u8 taskId)
{
    u8 spriteId;
    ...
    if (!gPaletteFade.active)
    {
        ...
        gTasks[taskId].func = Task_NewGameFrankSpeech_Cleanup;
    }
}

static void Task_NewGameFrankSpeech_Cleanup(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        FreeAllWindowBuffers();
...
static void CB2_NewGameFrankSpeech_ReturnFromNamingScreen(void)
{
    ...
    DmaFill16(3, 0, PLTT, PLTT_SIZE);
    ResetPaletteFade();
    DecompressDataWithHeaderVram(sFrankSpeechShadowGfx, (u8 *)VRAM);
    DecompressDataWithHeaderVram(sFrankSpeechBgMap, (u8 *)(BG_SCREEN_ADDR(7)));
    LoadPalette(sFrankSpeechBgPals, BG_PLTT_ID(0), 2 * PLTT_SIZE_4BPP);
    LoadPalette(&sFrankSpeechBgGradientPal[1], BG_PLTT_ID(0) + 1, PLTT_SIZEOF(8));
    ResetTasks();
    taskId = CreateTask(Task_NewGameFrankSpeech_ReturnFromNamingScreenShowTextbox, 0);
...
static u8 NewGameFrankSpeech_CreateLotadSprite(u8 x, u8 y)
{
    return CreateMonPicSprite_Affine(SPECIES_RATTATA_ALOLA, FALSE, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}

static void AddFrankSpeechObjects(u8 taskId)
{
    u8 frankSpriteId;
    u8 lotadSpriteId;
    u8 brendanSpriteId;
    ...
    frankSpriteId = AddNewGameFrankObject(0x88, 0x3C, 1);
    gSprites[frankSpriteId].callback = SpriteCB_Null;
    gSprites[frankSpriteId].oam.priority = 0;
    gSprites[frankSpriteId].invisible = TRUE;
    gTasks[taskId].tFrankSpriteId = frankSpriteId;
    lotadSpriteId = NewGameFrankSpeech_CreateLotadSprite(100, 0x4B);
...
static void Task_NewGameFrankSpeech_FadeOutTarget1InTarget2(u8 taskId)
{
    int alphaCoeff2;
...
static void NewGameFrankSpeech_StartFadeOutTarget1InTarget2(u8 taskId, u8 delay)
{
    u8 taskId2;
    ...
    taskId2 = CreateTask(Task_NewGameFrankSpeech_FadeOutTarget1InTarget2, 0);
...
static void Task_NewGameFrankSpeech_FadeInTarget1OutTarget2(u8 taskId)
{
    int alphaCoeff2;
...
static void NewGameFrankSpeech_StartFadeInTarget1OutTarget2(u8 taskId, u8 delay)
{
    u8 taskId2;
    ...
    SetGpuReg(REG_OFFSET_BLDY, 0);
    gTasks[taskId].tIsDoneFadingSprites = 0;
    taskId2 = CreateTask(Task_NewGameFrankSpeech_FadeInTarget1OutTarget2, 0);
    gTasks[taskId2].tMainTask = taskId;
...
static void Task_NewGameFrankSpeech_FadePlatformIn(u8 taskId)
{
    ...
    else
    {
        ...
        LoadPalette(&sFrankSpeechBgGradientPal[gTasks[taskId].tPalIndex], BG_PLTT_ID(0) + 1, PLTT_SIZEOF(8));
    }
}

static void NewGameFrankSpeech_StartFadePlatformIn(u8 taskId, u8 delay)
{
    u8 taskId2;

    taskId2 = CreateTask(Task_NewGameFrankSpeech_FadePlatformIn, 0);
    gTasks[taskId2].tMainTask = taskId;
...
static void Task_NewGameFrankSpeech_FadePlatformOut(u8 taskId)
{
    if (gTasks[taskId].tDelayBefore)
    {
        gTasks[taskId].tDelayBefore--;
...
static void NewGameFrankSpeech_StartFadePlatformOut(u8 taskId, u8 delay)
{
    u8 taskId2;

    taskId2 = CreateTask(Task_NewGameFrankSpeech_FadePlatformOut, 0);
    gTasks[taskId2].tMainTask = taskId;
...
void NewGameFrankSpeech_SetDefaultPlayerName(u8 nameId)
{
    const u8 *name;
    u8 i;
...
static void NewGameFrankSpeech_ClearGenderWindowTilemap(u8 bg, u8 x, u8 y, u8 width, u8 height, u8 unused)
{
    FillBgTilemapBufferRect(bg, 0, x + 255, y + 255, width + 2, height + 2, 2);
}

static void NewGameFrankSpeech_ClearGenderWindow(u8 windowId, bool8 copyToVram)
{
    CallWindowFunction(windowId, NewGameFrankSpeech_ClearGenderWindowTilemap);
...
static void NewGameFrankSpeech_WaitForThisIsPokemonText(struct TextPrinterTemplate *printer, u16 renderCmd)
{
    // Wait for Frank's "This is a Pokémon" text to reach the pause
    ...
    if (*(printer->currentChar - 2) == EXT_CTRL_CODE_PAUSE && !sStartedPokeBallTask)
    {
        ...
        CreateTask(Task_NewGameFrankSpeechSub_InitPokeBall, 0);
    }
}
...
static void NewGameFrankSpeech_ShowDialogueWindow(u8 windowId, u8 copyToVram)
{
    CallWindowFunction(windowId, NewGameFrankSpeech_CreateDialogueWindowBorder);
    FillWindowPixelBuffer(windowId, PIXEL_FILL(1));
...
static void NewGameFrankSpeech_CreateDialogueWindowBorder(u8 bg, u8 x, u8 y, u8 width, u8 height, u8 palNum)
{
    FillBgTilemapBufferRect(bg, FRANK_DLG_BASE_TILE_NUM +  1, x-2,       y-1, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, FRANK_DLG_BASE_TILE_NUM +  3, x-1,       y-1, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, FRANK_DLG_BASE_TILE_NUM +  4, x,         y-1, width,   1, palNum);
    FillBgTilemapBufferRect(bg, FRANK_DLG_BASE_TILE_NUM +  5, x+width-1, y-1, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, FRANK_DLG_BASE_TILE_NUM +  6, x+width,   y-1, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, FRANK_DLG_BASE_TILE_NUM +  7, x-2,       y,   1,       5, palNum);
    FillBgTilemapBufferRect(bg, FRANK_DLG_BASE_TILE_NUM +  9, x-1,       y,   width+1, 5, palNum);
    FillBgTilemapBufferRect(bg, FRANK_DLG_BASE_TILE_NUM + 10, x+width,   y,   1,       5, palNum);

    FillBgTilemapBufferRect(bg, BG_TILE_V_FLIP(FRANK_DLG_BASE_TILE_NUM + 1), x-2,       y+height, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, BG_TILE_V_FLIP(FRANK_DLG_BASE_TILE_NUM + 3), x-1,       y+height, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, BG_TILE_V_FLIP(FRANK_DLG_BASE_TILE_NUM + 4), x,         y+height, width-1, 1, palNum);
    FillBgTilemapBufferRect(bg, BG_TILE_V_FLIP(FRANK_DLG_BASE_TILE_NUM + 5), x+width-1, y+height, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, BG_TILE_V_FLIP(FRANK_DLG_BASE_TILE_NUM + 6), x+width,   y+height, 1,       1, palNum);
}

static void Task_NewGameFrankSpeech_ReturnFromNamingScreenShowTextbox(u8 taskId)
{
