...
static void MenuPal_DrawChoices(u8 selection)
{
    ...
    for (i = 0; i < MENU_PAL_COUNT; i++)
        styles[i] = 0;
    styles[selection] = 1;

    // Clear the area first by drawing all options in default style
    FillWindowPixelRect(WIN_OPTIONS, PIXEL_FILL(1), 104, YPOS_MENUPAL + 1, 94, 16);
    
    // Draw the selected color name
    DrawOptionMenuChoice(colorNames[selection], GetStringRightAlignXOffset(FONT_NORMAL, colorNames[selection], 198), YPOS_MENUPAL, styles[selection]);
}
