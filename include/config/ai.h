...
#define SHOULD_SWITCH_HASBADODDS_PERCENTAGE                         80
#define SHOULD_SWITCH_ENCORE_STATUS_PERCENTAGE                      100
#define SHOULD_SWITCH_ENCORE_DAMAGE_PERCENTAGE                      80
#define SHOULD_SWITCH_CHOICE_LOCKED_PERCENTAGE                      100 // Only if locked into status move
#define SHOULD_SWITCH_ATTACKING_STAT_MINUS_TWO_PERCENTAGE           80
...
#define SHOULD_SWITCH_CURSED_PERCENTAGE                         80
#define SHOULD_SWITCH_CURSED_STATS_RAISED_PERCENTAGE            20
#define SHOULD_SWITCH_NIGHTMARE_PERCENTAGE                      60
...
#define AI_GOOD_SCORE_THRESHOLD                                 110 // Move scores above this threshold are considered "good" when deciding switching
...
#define SUCKER_PUNCH_CHANCE                                     25 // Chance for the AI to not use Sucker Punch if the player has a status move
#define SUCKER_PUNCH_PREDICTION_CHANCE                          25 // Additional chance for the AI to not use Sucker Punch if actively predicting a status move if SUCKER_PUNCH_CHANCE fails
...
#define RISKY_AI_CRIT_STAGE_THRESHOLD                           1   // Stat stages at which Risky will assume it gets a crit
...
#define AI_CONSERVE_TERA_CHANCE_PER_MON                         5 // Chance for AI with smart tera flag to decide not to tera before considering defensive benefit is this*(X-1), where X is the number of alive pokemon that could tera
#define AI_TERA_PREDICT_CHANCE                                  65 // Chance for AI with smart tera flag to tera in the situation where tera would save it from a KO, but could be punished by a KO from a different move.

// AI_FLAG_PP_STALL_PREVENTION settings
#define PP_STALL_DISREGARD_MOVE_PERCENTAGE                      75 // Detection chance per roll
#define PP_STALL_SCORE_REDUCTION                                30 // Score reduction if any roll for PP stall detection passes
...
#define ASSUME_STATUS_HIGH_ODDS                         95 // Chance for AI to see extremely likely moves for a pokemon to have, like Spore
#define ASSUME_STATUS_MEDIUM_ODDS                       85 // Chance for AI to see moderately likely moves for a pokemon to have, like Protect
#define ASSUME_STATUS_LOW_ODDS                          65 // Chance for AI to see niche moves a pokemon may have but probably won't, like Entrainment
#define ASSUME_ALL_STATUS_ODDS                          45 // Chance for the AI to see any kind of status move.
...
#define FRIENDLY_FIRE_RISKY_THRESHOLD             1 // AI_FLAG_RISKY acceptable number of hits to KO the partner via friendly fire
#define FRIENDLY_FIRE_NORMAL_THRESHOLD            2 // typical acceptable number of hits to KO the partner via friendly fire
#define FRIENDLY_FIRE_CONSERVATIVE_THRESHOLD      3 // AI_FLAG_CONSERVATIVE acceptable number of hits to KO the partner via friendly fire
// Counterplay on the assumption of opponents Protecting.
#define DOUBLE_TRICK_ROOM_ON_LAST_TURN_CHANCE    60 // both pokemon use Trick Room on turn Trick Room expires in the hopes both opponents used Protect to stall, getting a free refresh on the timer
#define TAILWIND_IN_TRICK_ROOM_CHANCE            60 // use Tailwind on turn Trick Room expires in the hopes both opponents used Protect to stall
